...
public class JobsService {
    ...
    private List<JobDto> callRealJobkoreaApi(String keywords, int limit) {
        try {
            String requestUrl = String.format(
                "%s?size=%d&keyword=%s&rbcd=10007&ob=2",
                apiUrl, limit, keywords
            );
        ...
        if (q == null || q.isBlank()) {
            ...
            filtered = pool.stream()
                .filter(j ->
                    ...
                    (j.getTags() != null && j.getTags().stream().anyMatch(
                        t -> t != null && t.toLowerCase().contains(term)
                    ))
                )
                .toList();
        }
    ...
    private List<JobDto> generateMockData(int limit) {
        ...
        for (int i = base.size(); i < limit; i++) {
            ...
            int salaryMin = (b.getSalaryMin() != null ? b.getSalaryMin() : 4000)
                + (idx % 5) * 100;
            int salaryMax = Math.max(
                salaryMin + 1000,
                (b.getSalaryMax() != null ? b.getSalaryMax() : 7000)
                    + (idx % 3) * 150
            );
            ...
            list.add(
                JobDto.builder()
                    ...
                    .matchScore(
                        Math.round((70 + (idx % 30)) * 10.0) / 10.0
                    )
                    .postedAt(
                        now.minusDays(idx % 10).minusHours(idx % 24)
                    )
